<div class="content">
	<ul class="buttonlist">
		<li>
			<li><a href="<?php
				$link = array('controller' => 'enquetes', 'action' => 'add');
				echo $this->Html->url($link); ?>" class="btn btn_book"><span>Adicionar</span></a></li>
		</li>
	</ul>
	<div class="contenttitle radiusbottom0">
		<h2 class="table"><span>Enquetes</span></h2>
	</div>
			...
			<tbody>
				<?php foreach ($enquetes as $enquete): ?>
					<tr class="gradeX">
						...
						<td class="center">
							<?php echo $this->Html->link(__('Editar'), array('action' => 'edit', $enquete['Enquete']['id'])); ?>
							<?php echo $this->Form->postLink(__('Apagar'), array('action' => 'delete', $enquete['Enquete']['id']), null, __('Você tem certeza que deseja apagar # %s?',$enquete['Enquete']['name'])); ?>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
